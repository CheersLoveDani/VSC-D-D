...
export class ShopEditorProvider extends BaseCustomTextEditorProvider {

	public static readonly viewType = 'dnd.shopEditor';
	...
	public async resolveCustomTextEditor(
		...
		const subscription = this.setupWebview(document, webviewPanel, {
			onMessage: async (message) => {
				const msg = message as { type: string; data?: unknown; path?: string; query?: string; requestId?: string; name?: string; level?: number };
				switch (msg.type) {
					case 'updateData':
	...
	private handleItemSearch(webviewPanel: vscode.WebviewPanel, query: string, requestId: string): void {
	...
	private handleGetItemInfo(webviewPanel: vscode.WebviewPanel, name: string, requestId: string): void {
		const compendium = CompendiumService.getInstance();
	...
	private async handleOpenItem(name: string, currentDoc: vscode.TextDocument): Promise<void> {
		if (!name.trim()) {
			return;
		}
		...
		const existingFile = await findExistingCustomFile(baseUri, sanitizedName, '.dnditem');
		if (existingFile) {
